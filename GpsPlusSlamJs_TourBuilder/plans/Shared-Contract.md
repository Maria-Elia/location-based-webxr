...
| D6 | `altitude` persisted but **not yet consumed** (framework floor-Y deferred). | In the live AR session "not consumed" means every stop is placed on the **visitor's floor** (the AR frame's own `local-floor` `y = 0` mapped through the alignment, not camera height minus a phone-height constant), not at world `y = 0`: GPS-world `y` is absolute altitude there, so `0` is the ellipsoid. The desktop preview's floor is `y = 0`, so it uses `0`. |
